...
class FilePreferencesDialog(QObject):
	def __init__(self, app_style, edit_ext_cmd, ev_font, dontask, ln_nr):
		QObject.__init__(self)
	...
	def run_dialog(self):
		...
		return (self.app_style, self.dlg.prd_font_app_btn.font().toString(), self.edit_ext_cmd, 
			self.ev_font, self.exit_dontask, self.show_ln_nr)

def show_preferences_dialog(app_style, edit_ext_cmd, ev_font, dontask,  ln_nr):
	d = FilePreferencesDialog(app_style, edit_ext_cmd, ev_font, dontask, ln_nr)
	return d.run_dialog()
